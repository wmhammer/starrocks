...
struct MinByAggregateData<TYPE_DECIMALV2, guard::Guard> {
    ...
    void reset() {
        buffer_result.clear();
        max = DecimalV2Value::get_max_decimal();
    }
};
...
struct MinByAggregateData<PT, DecimalPTGuard<PT>> {
    using T = RunTimeCppType<PT>;
    raw::RawVector<uint8_t> buffer_result;
    T max = get_max_decimal<T>();
    void reset() {
        buffer_result.clear();
        max = get_max_decimal<T>();
    }
};
...
struct MinByAggregateData<TYPE_DATETIME, guard::Guard> {
    raw::RawVector<uint8_t> buffer_result;
    TimestampValue max = TimestampValue::MAX_TIMESTAMP_VALUE;
    void reset() {
        buffer_result.clear();
        max = TimestampValue::MAX_TIMESTAMP_VALUE;
    }
};
...
struct MinByAggregateData<TYPE_DATE, guard::Guard> {
    raw::RawVector<uint8_t> buffer_result;
    DateValue max = DateValue::MAX_DATE_VALUE;
    void reset() {
        buffer_result.clear();
        max = DateValue::MAX_DATE_VALUE;
    }
};
...
template <PrimitiveType PT>
struct MinByAggregateData<PT, StringPTGuard<PT>> {
    raw::RawVector<uint8_t> buffer_result;
    raw::RawVector<uint8_t> buffer_max;
    int32_t size = -1;
    bool has_value() const { return size > -1; }
...
template <PrimitiveType PT>
struct MinByElement<PT, MinByAggregateData<PT>, StringPTGuard<PT>> {
    void operator()(MinByAggregateData<PT>& state, Column* col, size_t row_num, const Slice& right) const {
        if (!state.has_value() || state.slice_max().compare(right) > 0) {
            state.buffer_result.resize(col->serialize_size(row_num));
            col->serialize(row_num, state.buffer_result.data());
            state.buffer_max.resize(right.size);
    ...
    void operator()(MinByAggregateData<PT>& state, const char* buffer, size_t size, const Slice& right) const {
        if (!state.has_value() || state.slice_max().compare(right) > 0) {
            state.buffer_result.resize(size);
            memcpy(state.buffer_result.data(), buffer, size);
            state.buffer_max.resize(right.size);
...
template <PrimitiveType PT, typename State, class OP, typename T = RunTimeCppType<PT>, typename = guard::Guard>
class MinByAggregateFunction final
        : public AggregateFunctionBatchHelper<State, MinByAggregateFunction<PT, State, OP, T>> {
public:
    using InputColumnType = RunTimeColumnType<PT>;
    ...
    std::string get_name() const override { return "min_by"; }
};

template <PrimitiveType PT, typename State, class OP>
class MinByAggregateFunction<PT, State, OP, RunTimeCppType<PT>, StringPTGuard<PT>> final
        : public AggregateFunctionBatchHelper<State, MinByAggregateFunction<PT, State, OP, RunTimeCppType<PT>>> {
public:
    void reset(FunctionContext* ctx, const Columns& args, AggDataPtr __restrict state) const override {
        this->data(state).reset();
    }
